...
def idx_vocab(args, cfg, num_batch, he_kg_id, he_kg_emb):
    # PROC_DATA = load_files(cfg["DATASET"]["PROC_DATA"])
    # REL2IDX = load_files(cfg["DATASET"]["REL2IDX"])
    # AVOCAB2IDX = load_files(cfg["DATASET"]["AVOCAB2IDX"])
    IDX2VOCAB = load_files(cfg["DATASET"]["IDX2VOCAB"])
    # VOCAB2IDX = load_files(cfg["DATASET"]["VOCAB2IDX"])
    # KG_2hop = load_files(cfg["DATASET"]["KG_2hop"])
    # GLOVE = load_files(cfg["DATASET"]["GLOVE"])
    # GLOVE_ANS_CAND = load_files(cfg["DATASET"]["GLOVE_ANS_CAND"])

    he_kg_emb = torch.reshape(he_kg_emb, (-1, 300))
    he_kg_id = he_kg_id.flatten()

    # kg_node = np.delete(he_kg_id.cpu(), np.arange(1, he_kg_id.cpu().shape[-1], 2), axis=-1)

    # for i in range(he_kg_id.shape[1]):
    annotate = [IDX2VOCAB[he_kg_id[i]] for i in range(he_kg_id.shape[0])]

    # 删除关系和填充符
    nodes_annotate = [string for index, string in enumerate(annotate) if "__" not in string and "<unk>" not in string]
    # 给出删除元素的序号
    rel_unk_indexes = [index for index, string in enumerate(annotate) if "__" in string or "<unk>" in string]

    # 对应删除关系和填充符相应的idx和特征元素
    nodes_idx = np.delete(he_kg_id.cpu(), rel_unk_indexes, axis=0)
    nodes_emb = np.delete(he_kg_emb.cpu(), rel_unk_indexes, axis=0)

    # 删除重复元素
    unique_strings = []         # 存放无重复字符串，通过字符串检测重复
    duplicate_indexes = []
    unique_emb = []
    vocab_emb_ids = {}
    unique_emb_ids = 0
    for idx, string in enumerate(nodes_annotate):
        if string not in unique_strings:
            unique_strings.append(string)
            unique_emb.append(nodes_emb[idx, :])
            vocab_emb_ids[string] = unique_emb_ids
            unique_emb_ids += 1
        else:
            unique_emb[vocab_emb_ids[string]] += nodes_emb[idx, :]
            unique_emb[vocab_emb_ids[string]] /= 2
            duplicate_indexes.append(idx)

    unique_emb = torch.tensor([item.cpu().detach().numpy() for item in unique_emb]).cuda()

    tsne = TSNE(n_components=2, random_state=0)
    lastkq_3D = tsne.fit_transform(unique_emb[:, :].detach().cpu().numpy())

    io.savemat('hyperbolic_point.mat', {'x': lastkq_3D[:, 0], 'y': lastkq_3D[:, 1], 'text': unique_strings})
    # io.loadmat('point_3D.mat')['point_3D']
    # ax = plt.subplot(projection='3d')
    # plt.figure(dpi=100)
    # ax.scatter(lastkq_3D[:, 0], lastkq_3D[:, 1], lastkq_3D[:, 2], s=10, color='red')
    # for i in range(lastkq_3D.shape[0]):
    #     ax.text(lastkq_3D[i][0], lastkq_3D[i][1], lastkq_3D[i][2], unique_strings[i], fontdict=dict(fontsize=5))
...
class KVQA(Dataset):
    ...
    def __getitem__(self, idx):
        if self.args.model_name == "ggnn":
            ...
            #token查询实体
            # dict([val, key] for key, val in self.av2i.items())[6582]
            return ques_out, kg_out, ans
